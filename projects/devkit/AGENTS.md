...
DevKit is the merged ngx-prime demo/documentation app (showcase + uikit + translator):
an Angular 22 standalone, zoneless, OnPush, signals-based SPA under `projects/devkit/`.
This page-prompt contract applies to the showcase-derived pages mounted at the app root
(`projects/devkit/src/app/pages/**`).
...
- `PagePromptConfig` fields:
  ...
  - `sourcePath` — repo-relative path to the page's folder (e.g.
    `projects/devkit/src/app/pages/dashboards/default`).
